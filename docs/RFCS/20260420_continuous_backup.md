...
##### Mid-job span widening: a known v1 gap

In v1 the producers' rangefeed subscriptions are static —
fixed at job creation from `Scope.Spans(ctx, startHLC)` and
partitioned across SQL instances once. The coverage manifest
gets a new entry whenever the resolved span set changes, but
the producers don't pick up new spans created after startup.

This is a real gap, not "mostly avoided." Tables (not
databases) own keyspace in CRDB — a database is purely a
naming/parent reference, not a keyspace allocation. So:

- **CREATE TABLE inside a covered DB** allocates a fresh
  table ID from the cluster's next-table-id counter; its
  spans (`/Table/<new_id>/...`) live in keyspace none of
  the existing producer subscriptions cover. There is no
  "subscribe over the DB's keyspace" trick to pre-cover
  it — the DB has no keyspace.
- **CREATE INDEX on a covered table** allocates a fresh
  index ID under the existing table prefix
  (`/Table/<table_id>/<new_index_id>/...`). Whether the
  producer's subscription picks it up depends on whether
  `spansForAllTableIndexes` returned a tightly-fitted span
  per-index or one that covers the table's whole prefix.
  In practice the merged spans don't reserve keyspace for
  future indexes, so this is also typically uncovered.

Events in the gap are lost until the next BACKUP creates a
fresh revlog with the now-current resolved span set (see
"Lifecycle coupling to BACKUP runs" below). The bound on
how long the gap can last is the BACKUP cadence — schedules
running every few minutes get few-minute-bounded gaps;
manual one-off BACKUPs can leave indefinite gaps.

A future iteration adds **dynamic re-subscription**: the
coordinator detects span widening and either restarts the
flow with the broader span set (along the lines of the
crash-resume protocol — flushorder bumped to
`max(prior) + 1` so per-key ordering survives) or adds a
sidecar rangefeed for newly-in-scope spans. Either path is
out of scope for v1.

The earlier RFC sketched a flow-cancel-and-restart protocol
on every coverage change. Building it would have meant
significant machinery (cancel flow, drain producers' open
ticks, replan, restart with bumped flushorders) for the
narrow win of in-job widening. We deferred it; the gap is
...
- **Coverage gap between BACKUP runs across an identity
  swap.** Between when a DROP + CREATE happens and when the
  next BACKUP run creates a fresh revlog, there's a window
  where the new descriptor's data is being written but no
  revlog is watching it. Restore to a time in that window
  is served only by the most-recent BACKUP, not by
  log-replay extending it. Acceptable for v1: the property
