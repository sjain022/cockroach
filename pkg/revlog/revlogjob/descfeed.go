...
func (s *descFeedState) handleValue(ctx context.Context, v *kvpb.RangeFeedValue) error {
	...
	// Coverage transitions are written only when the resolved span
	// set is narrowing (or unchanged in shape). Widening events
	// would claim coverage we don't have: producer subscriptions
	// are static at job creation, so newly-in-scope spans get a
	// schema delta but no producer is actually capturing their
	// data. Recording the wider coverage anyway would mislead
	// RESTORE into looking for data files that don't exist.
	// See "Mid-job span widening" in the RFC.
	if !s.lastSpansSet || (!sameSpans(s.lastSpans, newSpans) && spansSubset(newSpans, s.lastSpans)) {
